...
`P08` is the only criterion not at `pass`. The CI badge is now served by GitHub's
own workflow endpoint, so the one required value that has a first-party live
image no longer comes from a third party. Licence, platform, and release still
do, and committing generated images for them would trade a live value for a
stale one — the staleness problem badges exist to avoid. The standard's "where
practical" does not say which of the two it wants, so there is no work that
clears this criterion as written. [trsdn/.github#31](https://github.com/trsdn/.github/pull/31)
makes the rule turn on how a value changes and closes it at `pass`. Recorded as
`partial` until that lands, rather than argued away.
